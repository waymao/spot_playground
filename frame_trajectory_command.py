...
def main():
    ...
    with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        try:
            ...
            while cmd != "":
                dx = dy = dyaw = 0.0
                split_result = cmd.split(" ")
                if split_result[0] == "dx":
                    dx = float(split_result[1])
                elif split_result[0] == "dy":
                    dy = float(split_result[1])
                elif split_result[0] == "dyaw":
                    dyaw = float(split_result[1])
                result = relative_move(dx, dy, math.radians(dyaw), options.frame,
